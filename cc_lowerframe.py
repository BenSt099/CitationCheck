...
class LowerFrame(CTkFrame):
    def __init__(self, master, **kwargs):
        ...
        innerFrame = CTkFrame(master=self, height=20)
        ...
        upperFrame = CTkFrame(master=innerFrame)
        lowerFrame = CTkFrame(master=innerFrame)
        ...
        label_github = CTkButton(lowerFrame, fg_color = "transparent", height=10, hover_color="#181818", text="Homepage", image=image_github, compound="left", font=CTkFont(family='times new roman 14 bold', size=13, weight="bold"), command=self.openGitHub)
        label_github.grid(row=0, column=0, padx=0, pady=0, sticky="ew")

        image_issue = CTkImage(Image.open("issue.png"), size=(18,18))
        label_issue = CTkButton(lowerFrame, fg_color = "transparent", height=10, hover_color="#181818", text="Issues", image=image_issue, compound="left", font=CTkFont(family='times new roman 14 bold', size=13, weight="bold"), command=self.openGitHubIssue)
